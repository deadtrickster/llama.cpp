...
// has_bias_or_scale: the fused mat-vec kernel indexes a bias per output column for batched MUL_MAT_ID,
// which a plain MUL_MAT's one-dimensional bias does not have; gate-only fusion is what the batched
// MUL_MAT path supports
static bool ggml_cuda_should_fuse_mul_mat_vec_q(const ggml_tensor * tensor, bool has_bias_or_scale) {
    ggml_tensor *       src0 = tensor->src[0];
    ggml_tensor *       src1 = tensor->src[1];
    const ggml_tensor * dst  = tensor;
    ...
    // MUL_MAT: batches up to MMVQ_MAX_BATCH_SIZE (a speculative verify batch is 2-4 tokens; the
    // shared expert of a MoE layer is 3 mat-vecs per token otherwise) but gate-only past batch 1
    if (tensor->op == GGML_OP_MUL_MAT && dst->ne[1] != 1 && has_bias_or_scale) {
        return false;
    }
...
static int ggml_cuda_try_fuse(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, int i) {
    ...
    for (ggml_op op : { GGML_OP_MUL_MAT, GGML_OP_MUL_MAT_ID }) {
        ...
        if (op == GGML_OP_MUL_MAT) {
            for (const bool with_bias : { false, true }) {
                ...
                if (ggml_cuda_should_fuse_mul_mat_vec_q(up_n, up_bias || gate_bias || up_scale || gate_scale)) {
        ...
        if (ggml_cuda_can_fuse(cgraph, i, { op, bias_op, op, bias_op, GGML_OP_GLU }, {})) {
            ...
            if (ggml_cuda_should_fuse_mul_mat_vec_q(up, false)) {
                ggml_cuda_mm_fusion_args_host fusion_data{};
                fusion_data.gate      = gate->src[0];
                fusion_data.glu_op    = ggml_get_glu_op(glu);
                fusion_data.glu_limit = ggml_get_op_params_f32(glu, 3);
    ...
    for (ggml_op op : { GGML_OP_MUL_MAT, GGML_OP_MUL_MAT_ID }) {
        ...
        for (const bool with_bias : { false, true }) {
            ...
            if (ggml_cuda_should_fuse_mul_mat_vec_q(mm_node, true)) {
