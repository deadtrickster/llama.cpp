...
// [state-buf] Byte buffer for KV/state payloads: preallocated, prefaulted,
// PINNED and pooled. Replaces std::vector<uint8_t>, whose resize() zero-fills
// pages that the state copy immediately overwrites, and whose fresh pages then
// fault during DMA.
//
// Measured on gfx1201, 1299 MiB device->host in 128 chunks:
//     cold pageable (what vector gives)        6.17 GiB/s
//     mmap + MAP_POPULATE (prefaulted)        11.06
//     mmap + POPULATE + mlock (pinned)        24.94   <- 4.0x
//     hipHostMalloc (reference)               24.78
// Hardware ceiling is ~26 GiB/s per card, ~45 aggregate across both.
//
// mlock is what buys the 4x: the driver can DMA straight into locked pages
// instead of staging through a bounce buffer. Buffers are recycled through a
// free list so the mmap/populate/mlock cost is paid once, not per checkpoint.
class common_state_buf {
    // [state-buf] Shared payload with copy-on-write.
    //
    // server_prompt_cache::alloc() stores `prompt.checkpoints` by COPY so the
    // cache and the slot can both hold them. Checkpoints are IMMUTABLE once
    // captured, so that copy duplicated gigabytes for no reason - 2.33 GiB for a
    // 57.6k conversation, measured by ckpt_probe. Sharing the payload makes the
    // copy a refcount bump; a writer detaches first, so the semantics callers
    // already rely on are unchanged.
    //
    // The payload itself is mmap + MAP_POPULATE + mlock and recycled through a
    // bounded free list: pinned pages let the driver DMA straight in, worth 4x
    // over the pageable pages std::vector handed out (6.17 -> 24.94 GiB/s on
    // gfx1201, against a ~26 GiB/s per-card ceiling).
    struct payload {
        uint8_t * ptr = nullptr;
        size_t    cap = 0;
        ~payload();
