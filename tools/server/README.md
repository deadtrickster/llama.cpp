...
| `--seq-max N` | number of sequence ids the context is allocated for, must be >= --parallel (default: 0, 0 = same as --parallel)<br/>this is a residency ceiling for sequences that outlive their slot; today no id above --parallel is handed out<br/>NOT FREE: every id in [0, N) is charged its full recurrent state at allocation, whether used or not<br/>(hybrid/recurrent models; GLM-5.3-Flash: ~437 MiB per id with MTP rollback rows, ~146 MiB without)<br/>and without --kv-unified the KV cache is split into N streams, so per-slot context shrinks to n_ctx/N<br/>(env: LLAMA_ARG_SEQ_MAX) |
