...
| `--seq-max N` | number of sequence ids the context starts with, and the most it may grow to; must be >= --parallel (default: 0, 0 = start at --parallel, grow up to the library maximum)<br/>sequences outlive their slot: with --kv-unified the server raises the ceiling by one whenever a new conversation needs an id and the loaded model says one more fits, and lowers it again when idle<br/>NOT FREE: every id below the ceiling is charged its full recurrent state, whether used or not<br/>(hybrid/recurrent models; GLM-5.3-Flash: ~437 MiB per id with MTP rollback rows, ~146 MiB without)<br/>and without --kv-unified the KV cache is split into N streams, so per-slot context shrinks to n_ctx/N and the ceiling cannot move<br/>(env: LLAMA_ARG_SEQ_MAX) |
| `--seq-max-headroom MiB` | device memory to leave free when raising the sequence ceiling at runtime (default: 0)<br/>the raise is refused when a device would have less than this left after paying for one more sequence<br/>(env: LLAMA_ARG_SEQ_MAX_HEADROOM) |
| `--parallel-max N` | the most server slots there may be (default: 0, 0 = derive: as many as the pool holds sequences, up to the sequence ceiling's cap)<br/>slots follow residency: --parallel is the floor that always exists, and a slot is added for every resident sequence with work while the model says one more fits;<br/>set this only to hold the batch narrower than the pool - a yielded generation then stays resident and waits for a slot instead of being copied out<br/>(env: LLAMA_ARG_N_PARALLEL_MAX) |
| `--slot-deadline-preempt`, `--no-slot-deadline-preempt` | [deadline] make --slot-resume-after a hard bound: a suspended generation that has waited that long takes a seat (and, if it needs them, an id and KV room) from the running generation that has had its seat the longest, with a copy of that one's state (default: disabled). Off, it only wins a seat that frees up on its own<br/>(env: LLAMA_ARG_SLOT_DEADLINE_PREEMPT) |
| `--decode-per-prefill N` | [ratio] while a prompt is being prefilled and other slots are generating, run N decode-only batches between two prefill batches (default: 0, 0 = merge prefill and decode into every batch). Merged, a generating slot gets ONE token per prefill batch, so its rate becomes the prefill's batch rate (measured on GLM: 0.34 t/s against 43 during a 174k-token prefill); N gives decode its own clock at the price of N small batches per prefill batch<br/>(env: LLAMA_ARG_DECODE_PER_PREFILL) |
