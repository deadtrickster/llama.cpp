...
// server_prompt_cache
//
// [l2-spill] level-2 (disk) tier for the prompt cache.
//
// Without this, server_prompt_cache::update() destroys evicted entries
// (states.pop_front()) and the conversation must be fully reprocessed the next
// time it is seen -- ~380 s for a 130k-token prompt on this hardware, versus
// a few seconds to read it back. See the upstream TODO in server-context.cpp
// ("move slot to level 2 cache instead of removing?").
//
// Only the bulk buffers are written out. prompt.tokens and the checkpoint
// position metadata stay in RAM, so the prefix matching in load() is unchanged
// and never touches the disk -- an index entry for a 130k conversation is
// ~520 KB, so thousands of conversations can be indexed while their KV lives
// on NVMe.
