...
| three conversations on one pool | `9b3adc72c` `8cd0a4508` `f72e89d7e` `050163efa` `48ad98b2a` | under pressure a lone generation got a 500 and slot files, split draft groups, recurrent shrinks and newcomers lost state | pressure suspends and restores, never answers 500 |
| a cached conversation comes back beside a running one | `364a6eccb` `f5a82a91e` `5e8cf9f05` | a restore prefilled 115k tokens instead of suspending as the batch did; a resume's id was shrunk away, server aborted | restore in seconds instead of prefill in minutes, no crash on resume |
